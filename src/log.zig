...
pub const Logger = struct {
    ...
    pub fn writeMetrics(self: Logger, w: *std.Io.Writer) !void {
        try w.print("t,spikes,exc_spikes,inh_spikes,mean_u,exc_current,inh_current,scheduled_events,mean_threshold,mean_rate_ema,workspace_state,workspace_winner\n", .{});
        for (self.steps.items) |m| {
            try w.print("{d},{d},{d},{d},{d:.5},{d:.5},{d:.5},{d},{d:.5},{d:.6},{d:.5},{d}\n", .{
                m.t,
                m.spikes,
                m.exc_spikes,
                m.inh_spikes,
